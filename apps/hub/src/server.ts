...
import { assertSafeConfig, type HubConfig } from "./config.js";
import { trustFunction } from "./net.js";
...
export async function createServer(config: HubConfig): Promise<HubServer> {
  ...
  const app = Fastify({
    ...
    // Never `true`: only the configured hop count / proxy addresses may set the client IP (H3).
    trustProxy: trustFunction(config.trustProxy),
    bodyLimit: 64 * 1024,
  });
  ...
  registerHttpRoutes(app, {
    store,
    onUserRegistered: (user) => gateway.userUpserted(user),
    onSessionsRevoked: (userId, opts) => gateway.revokeSessions(userId, opts),
  });
